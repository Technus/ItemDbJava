...
public class PrintQR {
    ...
    public static BufferedImage get(DiscriminatedObjectId objectId) {
        try {
            ...
            BufferedImage target=new BufferedImage(imageL.getWidth(null)*5/3,imageL.getHeight(null),image.getType());
            Graphics g=target.getGraphics();
            g.setFont(new Font("Consolas", Font.BOLD, 52));
            g.setColor(Color.WHITE);
            g.fillRect(0,0,target.getWidth(),target.getHeight());
            g.drawImage(imageL,2,0,imageL.getWidth(null),imageL.getWidth(null),null);
            g.setColor(Color.BLACK);
            g.drawString(""+objectId.discriminator,
                    imageL.getWidth(null)+((target.getWidth()-imageL.getWidth(null))-g.getFontMetrics().stringWidth(""+objectId.discriminator))/2,
    ...
    public static void print(DiscriminatedObjectId objectId) {
        try {
            ...
            if (printer == null) {
                for (PrintService printer : PrinterJob.lookupPrintServices()) {
                    if (printer.getName().contains(printerName.get())) {
                        ...
                        printJob.setPrintService(PrintQR.printer = printer);
                    }
                }
            } else {
            ...
            PrintRequestAttributeSet attributesSet = new HashPrintRequestAttributeSet(new PrintRequestAttribute[]{
                    new PrinterResolution(203, 203, PrinterResolution.DPI),
    ...
    private static void printString(String args) throws Exception {
        ...
        BufferedImage image = MatrixToImageWriter.toBufferedImage(bm);
        PrinterJob printJob = PrinterJob.getPrinterJob();
        final Image imageL=image.getScaledInstance(image.getWidth()*2,image.getHeight()*2,Image.SCALE_FAST);
